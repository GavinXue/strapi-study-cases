...
class SDLConverter {
    ...
    printField(source, field) {
        const { namedType, wrapping } = wrapping_1.unwrapGraphQLDef(field.type);
        let prefix = 't.';
    ...
    printArg(arg) {
        ...
        const { namedType: type, wrapping } = wrapping_1.unwrapGraphQLDef(arg.type);
        const isArg = !graphql_1.isSpecifiedScalarType(type);
        let str = '';
        if (isArg) {
...
function isCommonScalar(field) {
    if (graphql_1.isScalarType(field)) {
        return field.name === 'UUID' || field.name === 'Date' || graphql_1.isSpecifiedScalarType(field);
    }
    return false;
}
