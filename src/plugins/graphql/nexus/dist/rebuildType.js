...
function rebuildScalarType(type, config) {
    return scalarType_1.scalarType(Object.assign(Object.assign({}, type.toConfig()), { sourceType: config.sourceType, asNexusMethod: config.asNexusMethod }));
}
exports.rebuildScalarType = rebuildScalarType;
function rebuildEnumType(type, { sourceType, asNexusMethod }) {
    const _a = type.toConfig(), { name, values } = _a, config = tslib_1.__rest(_a, ["name", "values"]);
    return enumType_1.enumType(Object.assign(Object.assign({ name }, config), { members: Object.entries(values).map(([valueName, config]) => {
...
function rebuildInterfaceType(type, config) {
    const { name, fields, description, interfaces, extensions, resolveType } = utils_1.graphql15InterfaceConfig(type.toConfig());
...
function rebuildOutputDefinition(typeName, t, fields, interfaces, config) {
    ...
    for (const [fieldName, fieldConfig] of Object.entries(fields)) {
        ...
        const { namedType, wrapping } = wrapping_1.unwrapGraphQLDef(fieldConfig.type);
        (_c = config.captureLeafType) === null || _c === void 0 ? void 0 : _c.call(config, namedType);
        t.field(fieldName, {
            type: wrapping_1.applyNexusWrapping(namedType.name, wrapping),
...
function rebuildArgs(typeName, fieldName, argMap, config) {
    ...
    for (const [argName, argConfig] of Object.entries(argMap)) {
        ...
        const { namedType, wrapping } = wrapping_1.unwrapGraphQLDef(argConfig.type);
        (_c = config.captureLeafType) === null || _c === void 0 ? void 0 : _c.call(config, namedType);
        rebuiltArgs[argName] = args_1.arg({
            type: wrapping_1.applyNexusWrapping(namedType.name, wrapping),
